...
impl<'a> Cx<'a> {
    ...
    fn emit_prologue(&mut self) {
        ...
        self.w("    OVERLAPPED ov = {0}; ov.Offset = (DWORD)(off & 0xFFFFFFFF); ov.OffsetHigh = (DWORD)(off >> 32);\n");
        self.w("    DWORD got = 0;\n");
        self.w("    if (!ReadFile(h, buf, (DWORD)n, &got, &ov)) return -1;\n");
        self.w("    return (ssize_t)got;\n");
        self.w("}\n");
        self.w("static inline ssize_t pwrite(int fd, const void* buf, size_t n, long off) {\n");
        self.w("    HANDLE h = (HANDLE)_get_osfhandle(fd);\n");
        self.w("    if (h == INVALID_HANDLE_VALUE) return -1;\n");
        self.w("    OVERLAPPED ov = {0}; ov.Offset = (DWORD)(off & 0xFFFFFFFF); ov.OffsetHigh = (DWORD)(off >> 32);\n");
        self.w("    DWORD wrote = 0;\n");
        self.w("    if (!WriteFile(h, buf, (DWORD)n, &wrote, &ov)) return -1;\n");
        self.w("    return (ssize_t)wrote;\n");
        self.w("}\n");
